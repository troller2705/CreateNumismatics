...
//			.generateBoth(tags, tag -> tag.add(AllItems.IRON_SHEET.get()));
    ...
    public static <T> void addTagToAllInRegistry(RegistrateTagsProvider<T> prov, DefaultedRegistry<T> defaultedRegistry, TagKey<T> tagKey) {
        prov.addTag(tagKey)
                .addAll(defaultedRegistry.keySet()
                        .stream()
                        .filter(id -> id.getNamespace().equals(Numismatics.MOD_ID))
                        .map(id -> ResourceKey.create(defaultedRegistry.key(), id))
                        .sorted(Comparator.comparing(rk -> rk.location().getPath()))
                        .toList()
                );
    }
}
