...
import dev.ithundxr.createnumismatics.content.salepoint.states.MultiloaderFluidStackImpl;
...
public class SalepointFluidDisplayWidget extends AbstractSimiWidget implements ISalepointStateUpdatingWidget {
    ...
    public static void renderFluidBox(@NotNull MultiloaderFluidStack fluidStack, float xMin, float yMin, float zMin,
        ...
        var fluidStackImpl = ((MultiloaderFluidStackImpl) fluidStack);
        var fluid = fluidStackImpl.getFluid();
        var fluidAmount = fluidStackImpl.getAmount();
        var isGas = fluidStackImpl.isLighterThanAir();

        FluidRenderer.renderFluidBox(fluid, fluidAmount, xMin, yMin, zMin, xMax, yMax, zMax, buffer, ms, light, renderBottom, isGas);
    }
}
